...
#if defined(NDEBUG)
# error "Tvcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xac9c720e60884bd294ac84fb2f8bdd1337df2516b38ce3e6008444f4217ab2bf");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tvcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tvcoin Signed Message:\n";
...
{
    // Tvcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 500000); // Tvcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Tvcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Tvcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tvcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tvcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3030414681ea90f02a6a074cd872222057953443fa5ca216803e7b09d41c9c77");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "TVcoingo";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b70e87dfe6a6143f625b58eaf6fc078a9e0615bfcefb726b4c57cc20087230f37f33c7c5272054779f1da0ee8d5ff2cdc1d7d1b0a3fe9f91f9f2c7228542578f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524892554;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085102348;

        if (fTestNet)
        {
            block.nTime    = 1524892522;
            block.nNonce   = 387455047;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x4134719bd357ca828dd3fc88e6eda659032abacf158584525229badd809922f1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xb1, 0xb2, 0xb3, 0xb4 }; // Tvcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TvcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TvcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TvcoinMiner(CWallet *pwallet)
{
    printf("TvcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tvcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TvcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TvcoinMiner, pwallet));
}
